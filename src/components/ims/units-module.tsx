...
// ============================================================================
// Electronics Mart IMS — Units Management UI
...
// ============================================================================
// CUSTOM HOOK: All Data Fetcher (with search support)
// ============================================================================

function useUnitsData(url: string) {
  const [data, setData] = React.useState<UnitRecord[]>([])
...
// ============================================================================
// UNIT VIEW DETAIL DIALOG
// ============================================================================

function UnitViewDialog({
  unit,
  open,
  onClose,
}: {
      ...
      <SheetContent side="right" className="sm:max-w-lg w-full p-0 overflow-hidden">
        <SheetHeader className="bg-slate-900 dark:bg-slate-800 text-white px-6 py-4 border-b border-slate-700">
          <SheetTitle className="text-white text-lg">
            {unit ? 'Edit Unit' : 'Create new'}
          </SheetTitle>
          <SheetDescription className="text-slate-300">
            {unit ? 'Update unit information' : 'Add a new unit record'}
          </SheetDescription>
        </SheetHeader>
          ...
          <div className="grid gap-4 p-6">
            ...
            <div className="space-y-1.5">
              ...
              <p className="text-[10px] text-muted-foreground">
                Short abbreviation displayed in monospace (e.g., pcs, kg, ltr, box, set)
              </p>
            </div>
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            ...
            ({unit?.code})? This is a soft-delete &mdash; the record will be marked as deleted
...
function ImportCSVDialog({
  ...
  const CSV_COLUMNS = ['name', 'symbol']
  const CSV_LABELS: Record<string, string> = {
    name: 'Name',
    symbol: 'Symbol (e.g., pcs, kg, ltr)',
  }
  ...
  const processMapping = () => {
    ...
    const symIdx = columnMap['symbol'] ? headers.indexOf(columnMap['symbol']) : -1
    ...
    for (let i = 1; i < rawRows.length; i++) {
      ...
      const symbol = symIdx >= 0 ? (row[symIdx] || '').trim() : ''
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-2xl max-h-[85vh]">
        <DialogHeader variant="navy">
          ...
          <DialogDescription>
            Upload a CSV file to bulk-import unit records
          </DialogDescription>
        </DialogHeader>
              ...
              <div className="space-y-4">
                ...
                >
                  ...
                  <p className="text-[11px] text-muted-foreground mb-3">
                    Required columns: name, symbol (optional)
                  </p>
                  <Input
                    type="file"
                    accept=".csv"
                    ...
                    <TableBody>
                      {staging.slice(0, 50).map((row, i) => (
                        <TableRow key={i}>
                          ...
                          <TableCell className="text-xs font-mono">{row.symbol || '\u2014'}</TableCell>
                        </TableRow>
                      ))}
                      {staging.length > 50 && (
                        <TableRow>
              ...
              <div className="space-y-4 text-center py-4">
                {importing ? (
                  <>
                    ...
                    <p className="text-sm text-muted-foreground">Importing units...</p>
                  </>
                ) : importResult ? (
                  <>
...
function exportUnitsCSV(units: UnitRecord[]) {
  const headers = ['Code', 'Name', 'Symbol', 'Products Count', 'Status', 'Created Date']
  const rows = units.map((u) => [
    u.code,
    u.name,
    u.symbol || '',
    String(u.productCount),
    u.isActive ? 'Active' : 'Inactive',
    u.createdDate ? formatDate(u.createdDate) : '',
  ])
  ...
  link.href = url
  link.download = `units_export_${new Date().toISOString().split('T')[0]}.csv`
  link.click()
  URL.revokeObjectURL(url)
  toast.success(`Exported ${units.length} units to CSV`)
}
...
function exportUnitsPDF(units: UnitRecord[]) {
  ...
  doc.text('Units Report', 14, 26)

  doc.setFontSize(9)
  doc.setTextColor(100)
  doc.text(`Generated: ${new Date().toLocaleString('en-GB')}`, 14, 32)
  doc.text(`Total Units: ${units.length}`, 14, 37)

  // Table data
  const tableData = units.map((u, idx) => [
    String(idx + 1),
    u.code,
    u.name,
    u.symbol || '\u2014',
  ...
  autoTable(doc, {
    startY: 42,
    head: [['Sl', 'Code', 'Name', 'Symbol', 'Products', 'Status']],
    body: tableData,
    theme: 'grid',
    styles: {
      fontSize: 9,
  ...
  doc.save(`units_report_${new Date().toISOString().split('T')[0]}.pdf`)
  toast.success(`Exported ${units.length} units to PDF`)
}
  ...
  return (
    <div className="flex flex-col items-center justify-center py-16 text-center">
      <div className="h-20 w-20 rounded-full bg-slate-50 dark:bg-slate-800/50 flex items-center justify-center mb-4">
        <Ruler className="h-10 w-10 text-slate-300 dark:text-slate-600" />
      </div>
...
export function UnitsModule() {
  ...
  const apiUrl = `/api/units${showInactive ? '?all=true' : ''}`
  const { data: units, loading, error, refetch } = useUnitsData(apiUrl)

  // All data for export (always fetch all including inactive for export)
  const { data: allUnits, refetch: refetchAll } = useUnitsData('/api/units?all=true')

  // UI state
  const [formOpen, setFormOpen] = React.useState(false)
  const [selectedUnit, setSelectedUnit] = React.useState<UnitRecord | null>(null)
  const [viewUnit, setViewUnit] = React.useState<UnitRecord | null>(null)
  const [viewOpen, setViewOpen] = React.useState(false)
  const [deleteUnit, setDeleteUnit] = React.useState<UnitRecord | null>(null)
  ...
  const handleSave = async (data: Record<string, unknown>) => {
    ...
    try {
      ...
      if (!res.ok) {
        const err = await res.json().catch(() => ({}))
        throw new Error(err.error || `Failed to ${isEdit ? 'update' : 'create'} unit`)
      }
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true)
    try {
      const res = await fetch(`/api/units?id=${deleteUnit.id}`, {
        method: 'DELETE',
      })
  ...
  const handleView = (unit: UnitRecord) => {
    setViewUnit(unit)
    setViewOpen(true)
  }

  const handleDeleteClick = (unit: UnitRecord) => {
    setDeleteUnit(unit)
    setDeleteOpen(true)
  }
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          <Ruler className="h-6 w-6 text-slate-900 dark:text-white" />
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
        ...
        <div className="flex items-center gap-2">
          <Label className="text-xs font-medium text-muted-foreground">Show Inactive</Label>
          <Switch
      ...
      {loading ? (
        ...
        <EmptyState hasFilters={!!searchTerm || showInactive} onClear={() => { setSearchTerm(''); setShowInactive(false); }} />
      ) : (
        <Card className="shadow-md">
              ...
              <Table>
                <TableHeader>
                  <TableRow className="bg-slate-50 dark:bg-slate-800/50 hover:bg-slate-50 dark:hover:bg-slate-800/50">
                    <TableHead className="text-xs font-semibold text-slate-700 dark:text-slate-300 w-12">Sl</TableHead>
                    <TableHead className="text-xs font-semibold text-slate-700 dark:text-slate-300">Code</TableHead>
                    <TableHead className="text-xs font-semibold text-slate-700 dark:text-slate-300">Name</TableHead>
                    <TableHead className="text-xs font-semibold text-slate-700 dark:text-slate-300">Symbol</TableHead>
                    <TableHead className="text-xs font-semibold text-slate-700 dark:text-slate-300 text-center">Products Count</TableHead>
                    <TableHead className="text-xs font-semibold text-slate-700 dark:text-slate-300">Status</TableHead>
                    <TableHead className="text-xs font-semibold text-slate-700 dark:text-slate-300 text-center">Actions</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {filteredUnits.map((unit, idx) => (
                    <TableRow key={unit.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30">
                      ...
                      <TableCell>
                        {unit.symbol ? (
                          <Badge className="bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-600 font-mono text-[11px] px-2 py-0.5">
                            {unit.symbol}
                          </Badge>
                        ) : (
