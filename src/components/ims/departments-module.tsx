...
// ============================================================================
// Electronics Mart IMS — Departments Management UI
...
// ============================================================================
// CUSTOM HOOK: All Departments Data Fetcher
// ============================================================================

function useAllDepartments(url: string) {
...
// ============================================================================
// DEPARTMENT VIEW DETAIL DIALOG
// ============================================================================

function DepartmentViewDialog({
  department,
  open,
  onClose,
}: {
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader variant="navy">
          ...
          <DialogDescription>
            Department Details &mdash; {department.code}
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-x-6 gap-y-3 text-sm">
            <div>
              ...
              <p className="font-mono font-medium mt-0.5">{department.code}</p>
            </div>
            <div>
              <span className="text-muted-foreground text-xs font-medium uppercase tracking-wider">Status</span>
              <div className="mt-0.5">{getStatusBadge(department.isActive)}</div>
            </div>
            <div>
            ...
            <div>
              ...
              <p className="font-medium mt-0.5">{formatDate(department.createdDate)}</p>
            </div>
            <div>
              <span className="text-muted-foreground text-xs font-medium uppercase tracking-wider">Last Updated</span>
              <p className="font-medium mt-0.5">{formatDate(department.updatedDate)}</p>
            </div>
          </div>
          ...
          {department.description && (
            <div>
              ...
              <p className="text-sm mt-0.5 leading-relaxed">{department.description}</p>
            </div>
          )}
        </div>
        <DialogFooter>
...
// ============================================================================
// DEPARTMENT FORM SHEET (ADD / EDIT)
// ============================================================================
      ...
      <SheetContent side="right" className="sm:max-w-lg w-full p-0 overflow-hidden">
        <SheetHeader className="bg-slate-900 dark:bg-slate-800 text-white px-6 py-4 border-b border-slate-700">
          <SheetTitle className="text-white text-lg">
            {department ? 'Edit Department' : 'Create new'}
          </SheetTitle>
          <SheetDescription className="text-white/70">
            {department ? 'Update department information' : 'Add a new department record'}
          </SheetDescription>
        </SheetHeader>
        ...
        <ScrollArea className="flex-1 h-[calc(100vh-160px)]">
          <div className="grid gap-4 p-6">
            ...
            <div className="space-y-1.5">
              ...
              <Input
                value={department ? department.code : 'Auto-generated on save'}
                disabled
                className="bg-muted text-muted-foreground font-mono text-sm cursor-not-allowed"
              />
              <p className="text-[10px] text-muted-foreground flex items-center gap-1">
                <AlertCircle className="h-3 w-3" />
                Auto-generated in format DEP-00001. Cannot be edited.
              </p>
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <Input
                ...
                placeholder="e.g. Sales, Management, IT, HR"
                autoFocus
              />
            </div>
            ...
            <div className="flex items-center justify-between rounded-lg border p-3">
              <div className="space-y-0.5">
                ...
                <p className="text-[11px] text-muted-foreground">
                  {form.isActive ? 'Department is currently active' : 'Department is currently inactive'}
                </p>
              </div>
              <button
                type="button"
    ...
    <AlertDialog open={open} onOpenChange={onClose}>
      <AlertDialogContent>
          ...
          <AlertDialogDescription>
            ...
            {hasEmployees && (
              <span className="block mt-2 text-rose-600 dark:text-rose-400 font-medium">
                ...
                This department has {department.employeeCount} employee(s). Please reassign employees before deleting.
              </span>
            )}
            {!hasEmployees && (
        ...
        <AlertDialogFooter>
          ...
          <AlertDialogAction
            onClick={onConfirm}
            disabled={deleting || !!hasEmployees}
...
function ImportCSVDialog({
  ...
  const CSV_COLUMNS = ['name', 'description']
  ...
  const downloadTemplate = () => {
    ...
    const sampleRow = 'Sales,Sales and marketing department'
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-2xl max-h-[85vh]">
        <DialogHeader variant="navy">
          ...
          <DialogDescription>
            Upload a CSV file to bulk-import department records
          </DialogDescription>
        </DialogHeader>
              ...
              <div className="space-y-4">
                ...
                <div className="flex gap-2 justify-end">
                  ...
                  >
                    ...
                    Import {staging.length} Departments
                  </Button>
                </div>
              </div>
            )}
              ...
              <div className="space-y-4 text-center py-4">
                {importing ? (
                  <>
                    ...
                    <p className="text-sm text-muted-foreground">Importing departments...</p>
                  </>
                ) : importResult ? (
                  <>
...
function exportDepartmentsCSV(departments: Department[]) {
  const headers = ['Code', 'Name', 'Description', 'Employees', 'Status', 'Created Date']
  ...
  link.click()
  URL.revokeObjectURL(url)
  toast.success(`Exported ${departments.length} departments to CSV`)
}
...
function exportDepartmentsPDF(departments: Department[]) {
  ...
  doc.text('Departments Report', 14, 26)

  doc.setFontSize(9)
  doc.setTextColor(100)
  doc.text(`Generated: ${new Date().toLocaleString('en-GB')}`, 14, 32)
  doc.text(`Total Departments: ${departments.length}`, 14, 37)

  // Table data
  const tableData = departments.map((d, idx) => [
    String(idx + 1),
    d.code,
    d.name,
    d.description || '\u2014',
  ...
  autoTable(doc, {
    head: [['Sl', 'Code', 'Name', 'Description', 'Employees', 'Status']],
    body: tableData,
    startY: 42,
    styles: {
      fontSize: 8,
    ...
    headStyles: {
      fillColor: [25, 42, 86], // navy #192A56 header
      textColor: 255,
      fontStyle: 'bold',
    },
    alternateRowStyles: {
  ...
  toast.success(`Exported ${departments.length} departments to PDF`)
}
...
export function DepartmentsModule() {
  ...
  // Build API URL
  const apiUrl = `/api/departments?all=${showInactive ? 'true' : ''}&search=${encodeURIComponent(debouncedSearch)}`

  // Fetch data
  const { data: allDepartments, loading, error, refetch } = useAllDepartments(apiUrl)

  // Client-side filtering (additional filter on already fetched data)
  const filteredDepartments = React.useMemo(() => {
    if (!debouncedSearch) return allDepartments
  ...
  const pagination: PaginationInfo = React.useMemo(() => {
    ...
    return { page, pageSize, total, totalPages: Math.min(totalPages, 9999) }
  ...
  React.useEffect(() => {
    setPage(1)
  }, [debouncedSearch, pageSize, showInactive])
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true)
    try {
      const res = await fetch(`/api/departments?id=${deleteDepartment.id}`, { method: 'DELETE' })
      if (!res.ok) {
        const err = await res.json().catch(() => ({}))
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">
            {filteredDepartments.length} of {allDepartments.length} departments
          </p>
        </div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <Button
            variant="outline"
            size="sm"
            onClick={() => exportDepartmentsCSV(filteredDepartments)}
          ...
          <Button
            variant="outline"
            size="sm"
            onClick={() => exportDepartmentsPDF(filteredDepartments)}
            disabled={filteredDepartments.length === 0}
            className="text-xs"
          >
      ...
      {filteredDepartments.length === 0 ? (
        <EmptyState hasFilters={!!search || showInactive} onClear={() => { setSearch(''); setShowInactive(false) }} />
      ) : (
        <Card className="shadow-md">
              ...
              <Table>
                <TableHeader>
                  <TableRow className="bg-slate-50 dark:bg-slate-900/30 hover:bg-slate-50 dark:hover:bg-slate-900/30">
                    ...
                    <TableHead className="text-xs font-semibold text-slate-700 dark:text-slate-300">Department Code</TableHead>
                    <TableHead className="text-xs font-semibold text-slate-700 dark:text-slate-300">Department Name</TableHead>
                    <TableHead className="text-xs font-semibold text-slate-700 dark:text-slate-300">Description</TableHead>
                    <TableHead className="text-xs font-semibold text-slate-700 dark:text-slate-300 text-center">Employees</TableHead>
                ...
                <TableBody>
                  {paginatedDepartments.map((dept, idx) => (
                    <TableRow key={dept.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-900/20">
                      ...
                      <TableCell>
                        <div className="flex items-center justify-center gap-1">
                          <Button
                            variant="ghost"
                            size="icon"
                            className="h-7 w-7 text-slate-500 hover:text-amber-600 hover:bg-amber-50 dark:hover:bg-amber-900/20"
                          ...
                          <Button
                            variant="ghost"
                            size="icon"
                            className="h-7 w-7 text-slate-500 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/20"
                          ...
                          <Button
                            variant="ghost"
                            size="icon"
                            className="h-7 w-7 text-slate-500 hover:text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-900/20"
      ...
      <DepartmentFormSheet
        open={formOpen}
        onClose={() => { setFormOpen(false); setEditingDepartment(null) }}
