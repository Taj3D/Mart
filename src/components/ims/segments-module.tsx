...
// ============================================================================
// Electronics Mart IMS — Segments Management UI
...
// ============================================================================
// CUSTOM HOOK: All Segments Data Fetcher
// ============================================================================

function useAllSegments(url: string) {
  const [data, setData] = React.useState<Segment[]>([])
...
// ============================================================================
// SEGMENT VIEW DETAIL DIALOG
// ============================================================================

function SegmentViewDialog({
  segment,
  open,
  onClose,
}: {
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader variant="navy">
          ...
          <DialogDescription>
            Segment Details &mdash; {segment.code}
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-4">
          ...
          {segment.description && (
            <div>
              ...
              <p className="text-sm mt-0.5 leading-relaxed">{segment.description}</p>
            </div>
          )}
        </div>
        <DialogFooter>
      ...
      <SheetContent side="right" className="sm:max-w-lg w-full p-0 overflow-hidden">
        <SheetHeader className="bg-slate-900 dark:bg-slate-800 text-white px-6 py-4 border-b border-slate-700">
          <SheetTitle className="text-white text-lg">
            {segment ? 'Edit Segment' : 'Create new'}
          </SheetTitle>
          <SheetDescription className="text-white/70">
            {segment ? 'Update segment information' : 'Add a new segment record'}
          </SheetDescription>
        </SheetHeader>
        ...
        <ScrollArea className="flex-1 h-[calc(100vh-160px)]">
          <div className="grid gap-4 p-6">
            ...
            <div className="space-y-1.5">
              ...
              <Input
                value={segment ? segment.code : 'Auto-generated on save'}
                disabled
                className="bg-muted text-muted-foreground font-mono text-sm cursor-not-allowed"
              />
              <p className="text-[10px] text-muted-foreground flex items-center gap-1">
                <AlertCircle className="h-3 w-3" />
                Auto-generated in format SEG-00001. Cannot be edited.
              </p>
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <Input
                ...
                placeholder="e.g. Premium, Economy, Commercial"
                autoFocus
              />
            </div>
            ...
            <div className="flex items-center justify-between rounded-lg border p-3">
              <div className="space-y-0.5">
                ...
                <p className="text-[11px] text-muted-foreground">
                  {form.isActive ? 'Segment is currently active' : 'Segment is currently inactive'}
                </p>
              </div>
              <button
                type="button"
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            ...
            ({segment?.code})? This is a soft-delete &mdash; the record will be marked as deleted
...
function ImportCSVDialog({
  ...
  const downloadTemplate = () => {
    ...
    const sampleRow = 'Premium,High-end products segment'
    const csv = [header, sampleRow].join('\n')
    const blob = new Blob([csv], { type: 'text/csv' })
    const url = URL.createObjectURL(blob)
    const a = document.createElement('a')
    a.href = url
    a.download = 'segments_import_template.csv'
    a.click()
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-2xl max-h-[85vh]">
        <DialogHeader variant="navy">
          ...
          <DialogDescription>
            Upload a CSV file to bulk-import segment records
          </DialogDescription>
        </DialogHeader>
              ...
              <div className="space-y-4">
                ...
                <div className="flex gap-2 justify-end">
                  ...
                  >
                    ...
                    Import {staging.length} Segments
                  </Button>
                </div>
              </div>
            )}
              ...
              <div className="space-y-4 text-center py-4">
                {importing ? (
                  <>
                    ...
                    <p className="text-sm text-muted-foreground">Importing segments...</p>
                  </>
                ) : importResult ? (
                  <>
...
function exportSegmentsCSV(segments: Segment[]) {
  const headers = ['Code', 'Name', 'Description', 'Products Count', 'Status', 'Created Date']
  const rows = segments.map((s) => [
    ...
    s.createdDate ? formatDate(s.createdDate) : '',
  ])
  ...
  link.href = url
  link.download = `segments_export_${new Date().toISOString().split('T')[0]}.csv`
  link.click()
  URL.revokeObjectURL(url)
  toast.success(`Exported ${segments.length} segments to CSV`)
}
...
function exportSegmentsPDF(segments: Segment[]) {
  ...
  doc.text('Segments Report', 14, 26)

  doc.setFontSize(9)
  doc.setTextColor(100)
  doc.text(`Generated: ${new Date().toLocaleString('en-GB')}`, 14, 32)
  doc.text(`Total Segments: ${segments.length}`, 14, 37)

  // Table data
  const tableData = segments.map((s, idx) => [
    String(idx + 1),
    s.code,
    s.name,
    s.description || '\u2014',
  ...
  autoTable(doc, {
    head: [['Sl', 'Code', 'Name', 'Description', 'Products', 'Status']],
    body: tableData,
    startY: 42,
    styles: {
      fontSize: 8,
  ...
  doc.save(`segments_report_${new Date().toISOString().split('T')[0]}.pdf`)
  toast.success(`Exported ${segments.length} segments to PDF`)
}
    ...
    <div className="flex flex-col items-center justify-center py-16 text-center">
      ...
      <p className="text-sm text-muted-foreground mb-4 max-w-sm">
        {hasFilters
          ...
          : 'Start by creating your first segment.'}
      </p>
      {hasFilters && (
...
// ============================================================================
// MAIN COMPONENT: SegmentsModule
// ============================================================================

export function SegmentsModule() {
  // Data fetching
  ...
  const apiUrl = React.useMemo(() => {
    ...
    return `/api/segments?${params.toString()}`
  }, [debouncedSearch])

  const { data: allSegments, loading, error, refetch } = useAllSegments(apiUrl)
  ...
  }, [filteredSegments.length, page, pageSize])
  ...
  }, [filteredSegments, pagination.page, pageSize])
  ...
  // View/Delete state
  const [viewSegment, setViewSegment] = React.useState<Segment | null>(null)
  const [deleteSegment, setDeleteSegment] = React.useState<Segment | null>(null)
  ...
  const handleSave = async (data: Record<string, unknown>) => {
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(err.error || `Failed to ${isEdit ? 'update' : 'create'} segment`)
      }

      toast.success(isEdit ? 'Segment updated successfully' : 'Segment created successfully')
      setFormOpen(false)
  ...
  const handleDelete = async () => {
    ...
    try {
      setDeleting(true)
      const res = await fetch(`/api/segments?id=${deleteSegment.id}`, { method: 'DELETE' })
      if (!res.ok) {
        const err = await res.json().catch(() => ({ error: 'Failed' }))
  ...
  const handleExportCSV = () => {
    ...
    exportSegmentsCSV(filteredSegments)
  }
  ...
  const handleExportPDF = () => {
    ...
    exportSegmentsPDF(filteredSegments)
  }
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">
            Manage product segments and categories
          </p>
        </div>
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
        ...
        <span className="text-xs text-muted-foreground ml-auto">
          {filteredSegments.length} of {allSegments.length} segments
        </span>
      </div>
              ...
              <Table>
                <TableHeader>
                  <TableRow className="bg-slate-50 dark:bg-slate-900/30 hover:bg-slate-50 dark:hover:bg-slate-900/30">
                    ...
                    <TableHead className="text-xs font-semibold text-slate-700 dark:text-slate-300 w-[120px] text-center">Products</TableHead>
                ...
                <TableBody>
                  {paginatedSegments.map((segment, idx) => {
                    ...
                    return (
                      <TableRow key={segment.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-900/20">
                        <TableCell className="text-xs text-muted-foreground font-medium">{sl}</TableCell>
                        <TableCell className="font-mono text-xs font-medium text-slate-600 dark:text-slate-400">{segment.code}</TableCell>
                        <TableCell className="text-sm font-medium">{segment.name}</TableCell>
                        <TableCell className="text-sm text-muted-foreground" title={segment.description || undefined}>
                          {truncateDescription(segment.description, 40)}
                        </TableCell>
                        <TableCell className="text-sm text-center font-medium">{segment.productCount}</TableCell>
                        <TableCell className="text-center">{getStatusBadge(segment.isActive)}</TableCell>
                        <TableCell>
